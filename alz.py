...
SELECT
	...
	COALESCE (
		...
		CASE
				...
				WHEN STATUS NOT IN ( 200, 302 ) THEN
				1 
				WHEN path = '/login' 
				AND method = 'POST' 
				AND STATUS != 302 THEN
			...
			COALESCE (
				SUM( CASE WHEN path = '/login' AND method = 'POST' AND STATUS != 302 THEN 1 ELSE 0 END ) / NULLIF( COUNT( CASE WHEN path = '/login' AND method = 'POST' THEN 1 ELSE NULL END ), 0 ),
				0 
